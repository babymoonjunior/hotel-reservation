...
export default function ProfileUP({ session }) {
  ...
  const [formError, setFormError] = useState(null);
    ...
    useEffect(() => {
      // Fetch the user's data and set it as default values when available
  ...
  const onSubmit = async (data) => {
    ...
    if (!fullName || !email || !idNumber || !dateBirth || !country || !avatar_url) {
    ...
    try {
      ...
      const { error } = await supabase
        .from("profiles")
        .update({ full_name: fullName, email, id_card: idNumber, birthdate: dateBirth, country })
        .eq("id", user?.id)
        .single();
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="relative max-w-[1092px] h-[1000px] mx-auto pl-[50px] pr-[50px] pt-[50px]
